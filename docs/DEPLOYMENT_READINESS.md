...
`ENABLE_IMMICH_WEEK_HOVER` 是独立的渐进启用能力，代码默认保持 `0`；经用户
明确授权和生产验证后，当前 NAS 运行配置已设为 `1`。
启用“往年今日”不会开启悬停查询；点击周详情内的个人照片回放复用已审阅的
`ENABLE_IMMICH_MEMORIES` 只读能力，并且只在用户明确打开一周时查询。
应用侧悬停实现会等待 600ms，只为已关联 Immich 人物的成员在桌面精细指针上
显示一张代表照片，并在页面内最多缓存 64 个成员/周结果。其他环境部署仍须
单独审阅其 NAS 运行配置后才能把该开关设为 `1`。
