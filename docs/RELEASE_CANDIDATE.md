...
- 生产人物 onboarding 已启用，但家庭数据库仍为空；首次选择和建档必须由
  家庭成员在局域网页面中确认。
- 照片时间线、悬停回忆和“那年今日”尚未实现；页面只承诺已上线的人物导入。
- 前端仍依赖 Tailwind、daisyUI、GSAP 和 Flatpickr CDN。家庭网络可正常使用，
  但完全断网时的样式与交互离线化仍是后续工作。
...
- 在家庭局域网页面选择要导入的成员，并补齐缺失出生日期；
- 验证成员切换、家庭设置、事件保存和服务重启后的持久化；
